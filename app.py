...
with tab_single:
    ...
    if st.session_state.briefs:
        ...
        if sk and sk.get("clusters"):
            st.markdown("### 🧩 Mots-clés secondaires")
            for c in sk["clusters"]:
                placement = f" — *{c['placement']}*" if c.get("placement") else ""
                st.markdown(f"**{c.get('theme', '')}**{placement}")
                st.markdown("`" + "`  `".join(c.get("keywords", [])) + "`")
        ...
        for idx_bulk, item in enumerate(st.session_state.bulk_briefs):
            if item["status"] == "success":
                with st.expander(f"✅ {item['keyword']} — {item['word_range']} mots"):
                    ...
                    b_sk = item.get("secondary_kw")
                    if b_sk and b_sk.get("clusters"):
                        st.markdown("**🧩 Mots-clés secondaires**")
                        for c in b_sk["clusters"]:
                            placement = f" — *{c['placement']}*" if c.get("placement") else ""
                            st.markdown(
                                f"- **{c.get('theme', '')}**{placement} : "
                                + ", ".join(c.get("keywords", []))
                            )
