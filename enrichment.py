...
def generate_secondary_keywords(
    ...
    """
    Génère une LISTE UNIQUE de mots-clés secondaires (5 à 10 au total) liés au
    mot-clé principal, pour l'ensemble du contenu (pas de regroupement par section).

    Retour :
    {
      "keywords": ["mot-clé 1", "mot-clé 2", ...]   # 5 à 10 éléments
    }
    En cas d'erreur : {"error": "...", "keywords": []}.
    """
...
Plan (structure Hn) prévu pour l'article, à titre de contexte :
{outline}

Donne une LISTE UNIQUE de 5 à 10 mots-clés SECONDAIRES en {lang}, pour l'ENSEMBLE
de l'article (pas de regroupement par section). Ils doivent être directement liés
au mot-clé principal et couvrir le champ lexical / l'intention de recherche :
variantes, longue traîne, termes et entités connexes. Classe-les du plus
important au moins important. Pas de doublon, pas de simple répétition du mot-clé
principal.
...
N'invente pas de chiffres de volume de recherche."""

    try:
        raw = _call_claude(prompt, anthropic_key, model, max_tokens=800)
    except Exception as e:  # noqa: BLE001
        return {"error": f"Erreur API (mots-clés secondaires) : {e}", "keywords": []}

    data = _parse_json(raw)

    # Tolérance sur le format renvoyé par le modèle
    keywords = []
    if isinstance(data, dict) and "keywords" in data:
        ...
        # Rétro-compatibilité : on aplatit d'anciens clusters éventuels
    ...
    # Nettoyage : chaînes non vides, dédoublonnage insensible à la casse, plafond
    cleaned, seen = [], set()
    for k in keywords:
        k = str(k).strip()
        if k and k.lower() not in seen:
            seen.add(k.lower())
            cleaned.append(k)
    return {"keywords": cleaned[:max_keywords]}


def get_secondary_keywords_list(secondary_keywords: dict) -> list:
    """Renvoie la liste plate de mots-clés, en tolérant l'ancien format 'clusters'."""
    if not secondary_keywords:
        return []
    if secondary_keywords.get("keywords"):
        return list(secondary_keywords["keywords"])
    # Ancien format éventuel
    flat = []
    for c in secondary_keywords.get("clusters", []):
        if isinstance(c, dict):
            flat.extend(c.get("keywords", []))
    return flat
...
def enrichment_to_html(secondary_keywords: dict = None, internal_links: dict = None) -> str:
    ...
    kws = get_secondary_keywords_list(secondary_keywords or {})
    if kws:
        parts.append("<h2>Mots-clés secondaires</h2>")
        parts.append("<ul>")
        for k in kws:
            parts.append(f"<li>{_html.escape(k)}</li>")
        parts.append("</ul>")
...
def append_enrichment_to_docx(docx_buffer, secondary_keywords: dict = None,
    ...
    clusters = None  # (ancien format non utilisé)
    kws = get_secondary_keywords_list(secondary_keywords or {})
    links = (internal_links or {}).get("links", [])
    if not kws and not links:
        docx_buffer.seek(0)
        return docx_buffer
