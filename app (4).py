...
from enrichment import (
    ...
    get_secondary_keywords_list,
    generate_internal_linking,
    parse_manual_internal_links,
    fetch_site_urls,
    enrichment_to_html,
    append_enrichment_to_docx,
)
...
if "site_pages_cache" not in st.session_state:
    st.session_state.site_pages_cache = {}


def _get_cached_site_pages(url):
    """Récupère (et met en cache) la liste des pages du site cible via son sitemap."""
    if not url:
        return []
    cache = st.session_state.site_pages_cache
    if url not in cache:
        try:
            cache[url] = fetch_site_urls(url)
        except Exception:
            cache[url] = []
    return cache[url]
...
with st.sidebar:
    ...
    include_secondary_kw = st.checkbox(
        "Mots-clés secondaires",
        value=True,
        help="Génère des mots-clés secondaires liés au mot-clé principal, "
             "regroupés par thème, avec la section où les intégrer."
    )

    include_internal_linking = st.checkbox(
        "Maillage interne",
        value=True,
        help="Ajoute une section de maillage interne (ancre + lien, ou ancre seule)."
    )
    if include_internal_linking:
        maillage_mode = st.radio(
            "Définition des liens",
            ["✍️ Manuel (je définis les ancres/liens)", "🤖 Automatique (IA)"],
            index=0,
            help="En mode manuel, vous saisissez vous-même les ancres et les liens "
                 "(dans l'onglet Brief). En mode automatique, l'IA les propose à partir "
                 "du sitemap du site cible."
        )
        if maillage_mode.startswith("🤖"):
            max_internal_links = st.slider("Nb max de liens internes", 3, 12, 6)
            if not target_site_url:
                st.caption(
                    "💡 Renseignez l'URL du site cible pour obtenir des liens vers vos "
                    "pages existantes. Sans site, seules des ancres à créer seront proposées."
                )
        else:
    ...
    maillage_manuel = include_internal_linking and maillage_mode is not None and maillage_mode.startswith("✍️")

    st.markdown("---")
    st.markdown("### 🔍 Source concurrentielle")
    source_mode = st.radio(
        "Mode d'analyse",
        ["Top 10 SERP (DataForSEO)", "URLs manuelles"],
        index=0
    )
...
with tab_single:
    ...
    if source_mode == "URLs manuelles":
        st.markdown("#### URLs concurrentes (1 à 10)")
        manual_urls = []
        for i in range(10):
            url = st.text_input(
                f"URL {i+1}",
    ...
    # ── Maillage interne manuel (je définis mes ancres / liens) ───────────────
    manual_pairs = []
    if include_internal_linking and maillage_manuel:
        st.markdown("#### 🔗 Maillage interne — vos ancres / liens")
        st.caption(
            "Définissez vous-même les liens internes. Laissez le lien vide pour une "
            "**ancre seule** (contenu pas encore publié)."
        )
        maillage_input_method = st.radio(
            "Méthode de saisie",
            ["✍️ Tableau éditable", "📋 Coller (une ligne : ancre | lien)"],
            horizontal=True,
            key="maillage_input_method",
            label_visibility="collapsed",
        )

        if maillage_input_method.startswith("✍️"):
            edited = st.data_editor(
                ...
                column_config={
                    "Ancre": st.column_config.TextColumn(
                        "Ancre", help="Texte du lien", width="medium"),
                    "Lien (URL)": st.column_config.TextColumn(
                        "Lien (URL)", help="Laisser vide = ancre à créer", width="large"),
                },
            )
            for r in (edited or []):
                a = str(r.get("Ancre", "") or "").strip()
                u = str(r.get("Lien (URL)", "") or "").strip()
                if a:
                    manual_pairs.append({"ancre": a, "lien": u})
        else:
            pasted = st.text_area(
                "Collez vos liens (un par ligne)",
                height=160,
                placeholder="guide d'achat vélo | https://monsite.fr/guide-achat\n"
                            "entretien du vélo | https://monsite.fr/entretien\n"
                            "comparatif 2026        (ancre seule, sans lien)",
                key="manual_links_paste",
            )
            manual_pairs = pasted  # la chaîne est parsée telle quelle plus bas
    ...
    if generate_btn:
        ...
        if source_mode == "Top 10 SERP (DataForSEO)" and (not dataforseo_login or not dataforseo_password):
        ...
        if errors:
            ...
            with st.spinner("🔍 Analyse en cours..."):
                ...
                competitor_urls = []
                if source_mode == "Top 10 SERP (DataForSEO)":
                    progress.progress(10, text="Récupération du Top 10 SERP...")
                ...
                # Step 6: Enrichissement (mots-clés secondaires + maillage interne)
                lang_code = location_map[search_engine_location][1]
                brief_structure = brief.get("structure", []) if isinstance(brief, dict) else []
                brief_has_error = isinstance(brief, dict) and brief.get("error")

                secondary_kw = None
                if include_secondary_kw and not brief_has_error:
                    progress.progress(88, text="Génération des mots-clés secondaires...")
                    secondary_kw = generate_secondary_keywords(
                        keyword=keyword.strip(),
                        structure=brief_structure,
                        anthropic_key=anthropic_key,
                        language_code=lang_code,
                    )

                internal_links = None
                if include_internal_linking and not brief_has_error:
                    if maillage_manuel:
                        # L'utilisateur définit lui-même les ancres / liens
                        internal_links = parse_manual_internal_links(manual_pairs)
                    else:
                        progress.progress(94, text="Analyse du maillage interne...")
                        site_pages = _get_cached_site_pages(target_site_url)
                        internal_links = generate_internal_linking(
                            keyword=keyword.strip(),
                            structure=brief_structure,
    ...
    if st.session_state.briefs:
        ...
        # ── Mots-clés secondaires ────────────────────────────────────────────
        sk = latest.get("secondary_kw")
        sk_list = get_secondary_keywords_list(sk) if sk else []
        if sk_list:
            st.markdown("### 🧩 Mots-clés secondaires")
            st.caption("À répartir naturellement dans l'ensemble du contenu.")
            st.markdown("`" + "`  `".join(sk_list) + "`")
        elif sk and sk.get("error"):
            st.info(f"ℹ️ Mots-clés secondaires indisponibles : {sk['error']}")

        # ── Maillage interne ─────────────────────────────────────────────────
        il = latest.get("internal_links")
        if il and il.get("links"):
            st.markdown("### 🔗 Maillage interne")
            existing = [l for l in il["links"] if l.get("type") == "existing"]
            to_create = [l for l in il["links"] if l.get("type") != "existing"]
            if existing:
                st.markdown("**✅ Liens vers des pages existantes (ancre + lien)**")
                for l in existing:
                    section = f" · *{l['target_section']}*" if l.get("target_section") else ""
                    st.markdown(f"- [{l['anchor']}]({l['url']}) → {l['url']}{section}")
            if to_create:
                st.markdown("**🆕 Ancres à créer (contenu à produire, lien à venir)**")
                for l in to_create:
                    bits = [b for b in [l.get("target_section"), l.get("reason")] if b]
                    suffix = f" — {' · '.join(bits)}" if bits else ""
                    st.markdown(f"- **{l['anchor']}**{suffix}")
        elif il and il.get("error"):
            st.info(f"ℹ️ Maillage interne indisponible : {il['error']}")

        # ── Export ───────────────────────────────────────────────────────────
        st.markdown("### 📤 Exporter le brief")

        docx_buffer = export_to_docx(brief, latest)
        docx_buffer = append_enrichment_to_docx(
            docx_buffer, latest.get("secondary_kw"), latest.get("internal_links")
        )
        st.download_button(
        ...
        html_formatted += enrichment_to_html(
            latest.get("secondary_kw"), latest.get("internal_links")
        )
...
with tab_bulk:
    ...
    # Maillage interne manuel global (appliqué à tous les briefs)
    bulk_manual_links = ""
    if include_internal_linking and maillage_manuel:
        st.markdown("#### 🔗 Maillage interne manuel (commun à tous les briefs)")
        st.caption(
            "Ces liens seront ajoutés à chaque brief. Un par ligne, format « ancre | lien ». "
            "Lien vide = ancre seule (contenu à créer)."
        )
        bulk_manual_links = st.text_area(
            "Liens internes communs",
            height=140,
            placeholder="page pilier | https://monsite.fr/pilier\n"
                        "guide complet | https://monsite.fr/guide\n"
                        "sujet à créer",
            key="bulk_manual_links",
            label_visibility="collapsed",
        )
        ...
        if errors:
            ...
            for idx, item in enumerate(bulk_keywords):
                ...
                try:
                    # Get competitor URLs
                    if urls:
                        competitor_urls = urls[:10]
                    elif source_mode == "Top 10 SERP (DataForSEO)":
                        se_domain, lang, loc_code = location_map[search_engine_location]
                        serp_results = get_top_results(
                            kw, dataforseo_login, dataforseo_password,
                            se_domain=se_domain, language_code=lang,
                            location_code=loc_code, top_n=10
                        )
                        competitor_urls = serp_results.get("urls", [])
                    else:
                        competitor_urls = []
                    ...
                    # Enrichissement (mots-clés secondaires + maillage interne)
                    b_lang = location_map[search_engine_location][1]
                    b_secondary_kw = None
                    b_internal_links = None
                    if not brief.get("error"):
                        b_struct = brief.get("structure", [])
                        if include_secondary_kw:
                            b_secondary_kw = generate_secondary_keywords(
                                keyword=kw,
                                structure=b_struct,
                                anthropic_key=anthropic_key,
                        ...
                        if include_internal_linking:
                            if maillage_manuel:
                                # Maillage commun défini par l'utilisateur
                                b_internal_links = parse_manual_internal_links(bulk_manual_links)
                            else:
                                b_site_pages = _get_cached_site_pages(target_site_url)
                                b_internal_links = generate_internal_linking(
                                    keyword=kw,
                                    structure=b_struct,
                                    site_pages=b_site_pages,
                                    anthropic_key=anthropic_key,
                                    language_code=b_lang,
                                    max_links=max_internal_links,
                                )
                    ...
                    if brief.get("error"):
                        ...
                        st.session_state.bulk_briefs.append({
                            ...
                            "avg_words": int(avg_words),
                            "competitor_data": competitor_data,
                            "secondary_kw": b_secondary_kw,
                            "internal_links": b_internal_links,
                        })
        ...
        if success_count > 0:
            ...
            with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
                for item in st.session_state.bulk_briefs:
                    ...
                    buf = append_enrichment_to_docx(
                        buf, item.get("secondary_kw"), item.get("internal_links")
                    )
        ...
        for idx_bulk, item in enumerate(st.session_state.bulk_briefs):
            if item["status"] == "success":
                with st.expander(f"✅ {item['keyword']} — {item['word_range']} mots"):
                    ...
                    docx_buf = append_enrichment_to_docx(
                        docx_buf, item.get("secondary_kw"), item.get("internal_links")
                    )
                    kw_slug = re.sub(r'[^a-z0-9]+', '_', item["keyword"].lower()).strip('_')
                    st.download_button(
                        "📥 Télécharger .docx",
                    ...
                    # Aperçu enrichissement (mots-clés secondaires + maillage)
                    b_sk_list = get_secondary_keywords_list(item.get("secondary_kw"))
                    if b_sk_list:
                        st.markdown("**🧩 Mots-clés secondaires**")
                        st.markdown(", ".join(b_sk_list))
                    b_il = item.get("internal_links")
                    if b_il and b_il.get("links"):
                        st.markdown("**🔗 Maillage interne**")
                        for l in b_il["links"]:
                            if l.get("type") == "existing":
                                st.markdown(f"- [{l['anchor']}]({l['url']}) → {l['url']}")
                            else:
                                st.markdown(f"- **{l['anchor']}** _(ancre à créer)_")

                    # Copyable brief with components.html
                    html_bulk = export_to_html(b, item_meta)
                    html_bulk += enrichment_to_html(
                        item.get("secondary_kw"), item.get("internal_links")
                    )
                    import streamlit.components.v1 as components
                    components.html(f"""
